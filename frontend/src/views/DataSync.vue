<template>
  <div class="data-sync">
    <el-card class="box-card">
      ...
      <!-- 创建任务表单 -->
      <el-form :model="formData" label-width="100px" v-if="showForm">
        <el-row :gutter="20">
          <el-col :span="12">
        ...
        <el-form-item>
          <el-button type="primary" @click="submitTask" :loading="creating">创建任务</el-button>
...
const loadFrequencyOptions = async () => {
  try {
    ...
    if (res.data.code === 200) {
      frequencyOptions.value = res.data.data
    }
  } catch (error) {
...
const loadStockTypeOptions = async () => {
  try {
    ...
    if (res.data.code === 200) {
      stockTypeOptions.value = res.data.data
    }
  } catch (error) {
...
const refreshTask = async (task) => {
  ...
  try {
    ...
    if (res.data.code === 200) {
      ...
      if (index !== -1) {
        taskList.value[index] = res.data.data
      }
    }
  } catch (error) {
